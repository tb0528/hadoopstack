package com.xiaoxiaomo.hbase.pro.hdfs;
 ...
 * 一些简单的HDFS操作
 * 查看、上传、下载
 * Created by xiaoxiaomo on 2015/5/9.
 */
...
//        open(conf, fs);
    ...
    private static void listStatus(FileSystem fs) throws IOException {
        FileStatus[] listStatus = fs.listStatus(new Path("/in/shops/"));
        for (FileStatus listStatu : listStatus) {
            System.out.println(listStatu.getPath());
        }
    }
}
