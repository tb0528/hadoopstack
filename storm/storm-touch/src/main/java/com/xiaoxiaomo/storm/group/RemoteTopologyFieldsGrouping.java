 ...
 *
 */
public class RemoteTopologyFieldsGrouping {
	/**
	...
	public static class MySpout extends BaseRichSpout{
		...
		public void nextTuple() {
			...
			this.collector.emit(new Values(i,i%3));//负责把数据发射出去
			try {
				Thread.sleep(1000);
	...
	public static void main(String[] args) {
		...
		topologyBuilder.setBolt("bolt_1", new SumBolt(),2).fieldsGrouping("spout_1", new Fields("flag"));
		...
		if(args.length==0){
			...
			String simpleName = RemoteTopologyFieldsGrouping.class.getSimpleName();
			try {
				//把topology提交到真正的集群运行
